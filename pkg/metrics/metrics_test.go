...
func TestMetricsLoggerUnknownType(t *testing.T) {
	logger := NewMetricsLogger()
	logger.Start()
	defer logger.Close()

	// Test logging an unknown type (should not panic)
	logger.Log("unknown type")
	logger.Wait()

	// Should still have empty metrics
	desc := logger.ReadDesc()
	if desc != (DescMetrics{}) {
		t.Errorf("Expected empty DescMetrics after unknown type, got %v", desc)
	}

	transfer := logger.ReadTransfer()
	if transfer != (TransferMetrics{}) {
		t.Errorf("Expected empty TransferMetrics after unknown type, got %v", transfer)
	}
}
